...
@export var hand: Hand #relies on the hand 
...
func discard_cards() -> void: 
	...
	for card_ui in hand.get_children():
		...
		tween.tween_callback(hand.discarded_card.bind(card_ui))
...
func reshuffle_deck_from_discard() -> void:
	if not character.draw_pile.empty(): #only reshuffle if empty
		return
...
func _on_card_played(card: Card) -> void:
	character.discard.add_card(card) #played cards are added to discard pile
